...
public class FacebookSignInHandler extends SingleProviderSignInHandler<AuthUI.IdpConfig> {
    ...
    protected void onCreate() {
        AuthUI.IdpConfig config = getArguments();
        List<String> permissions = config.getParams()
    ...
    public void startSignIn(@NonNull FirebaseAuth auth,
        ...
        FlowParameters flowParams = activity.getFlowParams();
        if (flowParams == null) {
            reportMissingFlowParams(activity);
            WebDialog.setWebDialogTheme(R.style.FirebaseUI);
        } else {
            WebDialog.setWebDialogTheme(flowParams.themeId);
        }
        LoginManager.getInstance().logInWithReadPermissions(activity, mPermissions);
    }

    private void reportMissingFlowParams(@NonNull HelperActivityBase activity) {
        try {
            Log.e(TAG, "Missing FlowParameters. activity=" + activity.getClass().getName());
            try {
                ...
                crashlyticsClass.getMethod("setCustomKey", String.class, String.class)
                        .invoke(crashlytics, "firebaseui_activity", activity.getClass().getName());
                crashlyticsClass.getMethod("log", String.class)
                        .invoke(crashlytics, "FirebaseUI missing FlowParameters in FacebookSignInHandler; using default theme");
                crashlyticsClass.getMethod("recordException", Throwable.class)
                        .invoke(crashlytics,
                                new IllegalStateException("FirebaseUI missing FlowParameters in FacebookSignInHandler"));
            } catch (Throwable tr) {
